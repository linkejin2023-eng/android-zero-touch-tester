...
def find_source_zip(config, build_number, build_type, source_type, sku=None, remote_path=None):
    ...
    # --- 智慧路徑判定 ---
    # 如果路徑在本地存在 (開發者本地驗證模式)，則不加 SSH 前綴，讓 rsync 進行本地搬運
    if os.path.exists(zip_abs_path):
        final_zip_path = zip_abs_path
        final_json_path = json_abs_path
    else:
        final_zip_path = f"{user}@{host}:{zip_abs_path}"
        final_json_path = f"{user}@{host}:{json_abs_path}"
