...
send_smoke_test_report () {
    ...
    local mail_title="$status_tag[Thorpe_A15] Release Build: REL_$timestamp ($SKU/$variant_cap) - Smoke Test: $status"
    
    local win_root="\\\\\\\\10.192.188.16\\share\\\\thorpe\\\\Android_15\\\\Release_pega"
    local remote_path="${win_root}\\\\REL_${timestamp}\\\\${variant}"
...
trigger_remote_test () {
    ...
    # 異步執行測試，並在結束後立刻發信
    (
        # 1. Proactively delete old root summary file on remote server before triggering
